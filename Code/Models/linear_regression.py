...
def linear_regression_plot():
    ...
    :return:
    """
    wine_data = import_wine_data('winequality-red.csv')
...
def linear_regression_plot_together():
    """
    ...
    fig.tight_layout()
    plt.show()
    fig.savefig("lin_reg.pdf", fmt="pdf", bbox_inches='tight')
...
def poly_model():
    ...
    # fig = plt.figure()
    # ax = fig.add_subplot(1, 1, 1)
    # prediction, = ax.plot(x_name, prediction_values,'x')
    # actual, = ax.plot(x_name, targets, 'or')
    # ax.legend(
    #     [prediction, actual],
    #     ["Prediction",
